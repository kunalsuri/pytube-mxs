...
def get_throttling_function_name(js: str) -> str:
    ...
    function_patterns = [
        ...
        # r'a\.[a-zA-Z]\s*&&\s*\([a-z]\s*=\s*a\.get\("n"\)\)\s*&&\s*'                   # KSURI: OLD value
        # r'\([a-z]\s*=\s*([a-zA-Z0-9$]+)(\[\d+\])?\([a-z]\)',                          # KSURI: OLD value
        r'a\.[a-zA-Z]\s*&&\s*\([a-z]\s*=\s*a\.get\("n"\)\)\s*&&.*?\|\|\s*([a-z]+)',     # KSURI: Updated based on https://github.com/pytube/pytube/issues/1954
